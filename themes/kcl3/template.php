...
drupal_add_js('sites/all/themes/kcl3/js/jquery.ba-bbq.min.js'); 
drupal_add_js('sites/all/themes/kcl3/js/jquery.backgroundpos.js');
//drupal_add_js('sites/all/themes/kcl3/js/jquery.backgroundPosition.js');
//drupal_add_js('sites/all/themes/kcl3/js/jquery.timers-1.1.2.js'); 
drupal_add_js('sites/all/themes/kcl3/js/jquery-ui-1.8.17.custom.min.js');
drupal_add_js('sites/all/themes/kcl3/js/jquery.mousewheel.min.js');
drupal_add_js('sites/all/themes/kcl3/js/jquery.mCustomScrollbar.js'); 
drupal_add_js('sites/all/themes/kcl3/js/frontpage.js'); 
drupal_add_js('sites/all/themes/kcl3/js/slidemenu.js'); 
drupal_add_js('sites/all/themes/kcl3/js/soundmanager/script/soundmanager2-nodebug-jsmin.js'); 
 ...
 */
 ...
 function kcl3_nomarkup($variables) {
  $output = '';
  // Render the items.
...
function kcl3_breadcrumb($variables) {
 ...
 */
function kcl3_preprocess_page(&$vars) {
 ...
 */
function kcl3_preprocess_comment_wrapper(&$vars) {
 ...
 * @ingroup themeable
 */
function kcl3_menu_local_tasks() {
  return menu_primary_local_tasks();
}

function kcl3_comment_submitted($comment) {
...
function kcl3_node_submitted($node) {
  return t('!datetime ? !username',
    array(
...
function kcl3_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
...
*/
function kcl3_navigation_links($menu_name, $level = 0) {
  ...
  return kcl3_navigation_links_level($tree_page, $tree_all);
}
...
*/
function kcl3_navigation_links_level($tree_page, $tree_all) {
  $links = array();
...
*/
function kcl3_links($links, $attributes = array('class' => 'links')) {
  $output = '';
...
function kcl3_child_menu($menu = 'main-menu', $id) {

$path = 'node/'. $id;
$parent = menu_link_get_preferred($path);
$mlid = $parent['mlid'];
$tree = menu_tree_all_data($menu);
$subtree = kcl3_get_subtree($tree,$mlid);
 ...
 */
function kcl3_get_subtree($tree, $mlid) {
  ...
  foreach ($tree as $key => $element) {
    ...
    if ($mlid == $element['link']['mlid'])  {
      ...
      if ($element['below']) {
        $submatch = kcl3_get_subtree($element['below'], $mlid);
